    ...
    <header class="site-header bg-[#beffb4] border-b-2 border-[#358054] shadow-md sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center flex-wrap gap-4">
            ...
            <div class="flex items-center gap-3 sm:gap-4 flex-wrap justify-end">
                ...
                {{-- 1. BOTÃO VOLTAR AO MAPA (PÚBLICO) --}}
                ...
                {{-- 2. ÁREA ADMINISTRATIVA / LOGADA --}}
                @if(auth('admin')->check())
                    {{-- CENÁRIO: ADMINISTRADOR --}}
                    <div class="flex gap-2">
                        {{-- BOTÃO PAINEL (DESIGN IGUAL AO VOLTAR AO MAPA) --}}
                        <a href="{{ route('admin.dashboard') }}" class="btn bg-[#358054] text-white hover:bg-[#2d6e4b] px-4 py-2 rounded-lg font-bold shadow-sm transition text-sm sm:text-base flex items-center gap-1" title="Ir para o Painel Administrativo">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {{-- BOTÃO EDITAR (MANTIDO AZUL PARA DESTAQUE) --}}
                        <a href="{{ route('admin.about.edit') }}" class="bg-blue-600 text-white px-3 py-2 rounded-lg text-sm font-bold shadow hover:bg-blue-700 transition flex items-center gap-1" title="Editar esta página">
                ...
                @elseif(auth()->check())
                    ...
                    <div class="flex gap-2">
                        {{-- BOTÃO PAINEL (DESIGN IGUAL AO VOLTAR AO MAPA) --}}
                        <a href="{{ route('dashboard') }}" class="btn bg-[#358054] text-white hover:bg-[#2d6e4b] px-4 py-2 rounded-lg font-bold shadow-sm transition text-sm sm:text-base flex items-center gap-1" title="Ir para o Meu Painel">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
