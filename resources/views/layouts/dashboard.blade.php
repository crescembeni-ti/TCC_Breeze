...
<body class="font-sans antialiased flex flex-col min-h-screen">
    ...
    <div x-data="{ open: false }" class="flex flex-1">
        ...
        <aside :class="open ? 'translate-x-0' : '-translate-x-full'"
            ...
            <a href="{{ route('home') }}" class="sidebar-link"><i data-lucide="arrow-left-circle" class="icon"></i> Voltar ao Mapa</a>

            <!-- Logout Multi-guard -->
            @if (auth('admin')->check())
                <form method="POST" action="{{ route('admin.logout') }}" class="mt-2">
                    @csrf
                    <a href="#" class="sidebar-link logout-btn"><i data-lucide="log-out" class="icon"></i> Sair</a>
                </form>
            @elseif(auth('analyst')->check())
