...
@section('content')
    {{-- 
        X-DATA: Controla o Modal de Confirmação 
    --}}
    <div x-data="{ 
        showModal: false, 
            ...
            <form id="form-edit" method="POST" action="{{ route('admin.trees.update', $tree->id) }}" class="space-y-10">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('health_status', $tree->health_status) }}', 
                                selectedName: '{{ old('health_status', $tree->health_status) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        @foreach([
                            'cap' => 'CAP (cm)', 
                            'height' => 'Altura (m)', 
                            'crown_height' => 'Altura da Copa (m)', 
                            'crown_diameter_longitudinal' => 'Copa Longitudinal (m)', 
                            'crown_diameter_perpendicular' => 'Copa Perpendicular (m)', 
                            'total_width' => 'Largura Total (m)', 
                            'street_width' => 'Largura da Rua (m)', 
                            'gutter_height' => 'Altura da Gola (m)', 
                            'gutter_width' => 'Largura da Gola (m)', 
                            'gutter_length' => 'Comprimento da Gola (m)'
                        ] as $field => $label)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $label }}</label>
                            <input type="number" step="0.01" name="{{ $field }}" 
                                value="{{ old($field, $tree->$field) }}"
                                class="w-full border border-gray-300 rounded-lg shadow-sm px-3 py-2 bg-gray-50 text-gray-800 focus:ring-green-500 focus:border-green-500">
                        </div>
                        @endforeach
                    </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('bifurcation_type', $tree->bifurcation_type) }}', 
                                selectedName: '{{ old('bifurcation_type', $tree->bifurcation_type) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('stem_balance', $tree->stem_balance) }}', 
                                selectedName: '{{ old('stem_balance', $tree->stem_balance) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('crown_balance', $tree->crown_balance) }}', 
                                selectedName: '{{ old('crown_balance', $tree->crown_balance) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('organisms', $tree->organisms) }}', 
                                selectedName: '{{ old('organisms', $tree->organisms) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('target', $tree->target) }}', 
                                selectedName: '{{ old('target', $tree->target) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('injuries', $tree->injuries) }}', 
                                selectedName: '{{ old('injuries', $tree->injuries) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                        ...
                        <div x-data="{ 
                                open: false, 
                                selected: '{{ old('wiring_status', $tree->wiring_status) }}', 
                                selectedName: '{{ old('wiring_status', $tree->wiring_status) ?: 'Selecione...' }}' 
                            }" class="relative w-full">
                ...
                <div class="flex flex-wrap gap-4 pt-6 border-t">
                    <button type="button" @click="openModal('save')"
                        class="bg-green-600 text-white text-lg rounded-md shadow-md hover:bg-green-700 hover:shadow-lg active:bg-[#38c224] active:scale-95 transition px-8 py-3">
                        Salvar Alterações
                    </button>

                    <button type="button" @click="openModal('delete')"
                        class="bg-red-50 text-red-600 border border-red-200 text-lg rounded-md shadow-sm hover:bg-red-100 transition px-8 py-3">
                        Excluir Árvore
                    </button>

                    <a href="{{ route('admin.trees.index') }}" 
                        class="bg-gray-100 text-gray-700 text-lg rounded-md shadow-sm hover:bg-gray-200 transition px-8 py-3 decoration-0">
                        Voltar para Lista
                    </a>
                </div>
        ...
        <div x-show="showModal" 
             style="display: none;"
             class="fixed inset-0 bg-gray-900 bg-opacity-50 z-[9999] flex items-center justify-center backdrop-blur-sm"
            ...
            <div class="bg-white rounded-lg shadow-xl p-8 max-w-sm w-full text-center relative" @click.outside="showModal = false">
                ...
                <div class="flex justify-center gap-4">
                    <button @click="showModal = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 transition">
                        Cancelar
                    </button>
                    <button @click="confirmAction()" class="px-4 py-2 text-white rounded-lg font-semibold transition shadow-md" :class="confirmColor">
                        Confirmar
                    </button>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", async function() {
            ...
            try {
                ...
                console.error("Erro ao carregar bairros.json:", err);
            }
